...
function cueExists(logger) {
    var stdout = (0, system_1.runCommandSync)("cue", ["version"], logger).stdout;
...
function validateOutput(output, validateScriptPath, logger) {
    if (!cueExists(logger)) {
        console.warn(intl_1.intlMsg.commands_test_error_cueDoesNotExist());
    }
    var id = output.id, data = output.data, error = output.error;
    var index = id.lastIndexOf(".");
    var jobId = id.substring(0, index);
    var stepId = id.substring(index + 1);
    var selector = "".concat(jobId, ".\\$").concat(stepId);
    var jsonOutput = "".concat(TMPDIR, "/").concat(id, ".json");
    fs_1.default.writeFileSync(jsonOutput, JSON.stringify({ data: data, error: error === null || error === void 0 ? void 0 : error.message }, core_js_1.typesHandler, 2));
    var stderr = (0, system_1.runCommandSync)("cue", ["vet", "-d", selector, validateScriptPath, jsonOutput], logger).stderr;
