    ...
    try {
        for (var _b = __values(Object.keys(jobs)), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            if (jobId === "data" || jobId === "error") {
                throw Error("Reserved job name 'data' or 'error' found in job ".concat(idStack, ".").concat(jobId));
            }
            (0, exports.validateJobNames)(jobs[jobId].jobs, "".concat(idStack, ".").concat(jobId));
        }
    }
...
function loadValidationScript(manifestPath, cueFilepath) {
    ...
    if (!fs_1.default.existsSync(cueFilepath)) {
        console.error(intl_1.intlMsg.commands_test_error_validatorNotFound({
            path: cueFilepath,
        }));
        process.exit(1);
    }
...
function printJobOutput(output) {
    console.log("-----------------------------------");
    console.log("ID: ".concat(output.id));
    console.log("Job status: ".concat(output.status));
    if (output.data !== undefined) {
        console.log("Data: ".concat(JSON.stringify(output.data, core_js_1.typesHandler, 2)));
    }
    if (output.error) {
        console.log("Error: ".concat(output.error.message));
    }
    console.log("Validation status: ".concat(output.validation.status));
    if (output.validation.error !== undefined) {
        console.log("Validation error: ".concat(parseCmdError(output.validation.error)));
    }
    console.log("-----------------------------------");
}
...
var parseCmdError = function (error) {
    ...
    msgLines[1] = "".concat(msgLines[1].split(":").slice(1).join(":"));
