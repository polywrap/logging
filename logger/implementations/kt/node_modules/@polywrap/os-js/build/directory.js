...
function readDirectorySync(dir) {
    var importDirectoryEntry = function (root, dirent) {
        ...
        if (dirent.isDirectory()) {
            var entries_1 = (0, fs_1.readdirSync)(direntPath, {
                withFileTypes: true,
            })
            ...
            return {
                type: "File",
                name: dirent.name,
                data: (0, fs_1.readFileSync)(direntPath, { encoding: "utf-8" }),
            };
        }
    };
...
function writeDirectorySync(outputDir, dir, renderTemplate) {
    ...
    var outputDirectoryEntry = function (root, entry) {
        ...
        switch (entry.type) {
            case "File": {
                (0, file_1.writeFileSync)(entryPath, entry.data);
                break;
            }
            case "Directory": {
                try {
                    for (var _b = __values(entry.data), _c = _b.next(); !_c.done; _c = _b.next()) {
                        ...
                        if (!(0, fs_1.existsSync)(entryPath)) {
                            (0, fs_1.mkdirSync)(entryPath, { recursive: true });
                        }
            ...
            case "Template": {
                if (!renderTemplate) {
                    throw Error("outputDirectoryEntry: No renderTemplate function provided. Found template ".concat(entry.name));
                }
                (0, file_1.writeFileSync)(entryPath, renderTemplate(entry.data));
                break;
            }
            default: {
                throw Error("outputDirectoryEntry: Unknown entry type. Entry: ".concat(JSON.stringify(entry)));
            }
        }
    };
    try {
