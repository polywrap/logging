...
function resolveUseStatements(schema, schemaPath, abi) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            ...
            useCapture = /(?:#|""")*use[ \n\t]*{([a-zA-Z0-9_, \n\t]+)}[ \n\t]*for[ \n\t]*(\w+)[ \n\t]/g;
            keywords = __spreadArray([], __read(schema.matchAll(useKeywordCapture)), false);
            useStatements = __spreadArray([], __read(schema.matchAll(useCapture)), false);
            if (keywords.length !== useStatements.length) {
                throw Error("Invalid use statement found in file ".concat(schemaPath, ".\nPlease use one of the following syntaxes...\n").concat(types_1.SYNTAX_REFERENCE));
            }
            ...
            capabilitiesExt = [];
            parsedUses = (0, parse_1.parseUse)(useStatements);
            _loop_1 = function (parsedUse) {
                ...
                var capabilities = parsedUse.usedTypes
                    ...
                    return (0, schema_parse_1.createCapability)({ type: type, enabled: true });
                })
                    .reduce(function (o1, o2) { return (__assign(__assign({}, o1), o2)); });
                var interfaceType = (0, schema_parse_1.createInterfaceDefinition)({
                ...
                abi.interfaceTypes = abi.interfaceTypes
                    ? __spreadArray(__spreadArray([], __read(abi.interfaceTypes), false), [interfaceType], false) : [interfaceType];
            };
            try {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    keywords = __spreadArray([], __read(schema.matchAll(importKeywordCapture)), false);
                    externalImportStatements = __spreadArray([], __read(schema.matchAll(externalImportCapture)), false);
                    localImportStatements = __spreadArray([], __read(schema.matchAll(localImportCapture)), false);
                    totalStatements = externalImportStatements.length + localImportStatements.length;
                    if (keywords.length !== totalStatements) {
                        throw Error("Invalid import statement found in file ".concat(schemaPath, ".\nPlease use one of the following syntaxes...\n").concat(types_1.SYNTAX_REFERENCE));
                    }
                    interfaceCapture = new RegExp("type\\s+".concat(TYPE_NAME_REGEX, "\\s+implements\\s([^{]*){"), "g");
                    implementInterfaceStatments = __spreadArray([], __read(schema.matchAll(interfaceCapture)), false);
                    implementationsWithInterfaces = parseInterfaces(implementInterfaceStatments);
                    externalImportsToResolve = (0, parse_1.parseExternalImports)(externalImportStatements);
                    localImportsToResolve = (0, parse_1.parseLocalImports)(localImportStatements);
                    subAbi = {
                        version: "0.1",
                        objectTypes: [],
                        enumTypes: [],
                    ...
                    return [4 /*yield*/, resolveExternalImports(schemaFile, externalImportsToResolve, abiResolver, subAbi)];
                case 1:
                    externalImports = _a.sent();
                    return [4 /*yield*/, resolveLocalImports(schemaFile, localImportsToResolve, abiResolver, subAbi)];
                case 2:
                    _a.sent();
                ...
                case 3:
                    ...
                    newSchema = schema_parse_1.header + newSchema + (0, render_1.renderSchema)(subAbi, false);
                    ...
                    newSchema = newSchema.replace(new RegExp("(type\\s+".concat(TYPE_NAME_REGEX, "[^{]*){\\s*}"), "g"), "$1");
                    // Parse and return the newly formed schema
                    return [2 /*return*/, (0, schema_parse_1.parseSchema)(newSchema, { noValidate: noValidate })];
            }
        });
    });
}
    ...
    var findImport = function (type, namespaceType, rootTypes, importedTypes, kind) {
        ...
        if (idx === -1) {
            throw Error("extractObjectImportDependencies: Cannot find the dependent type within the root type info.\n" +
                "Type: ".concat(type, "\nAbi: ").concat(JSON.stringify(rootAbi), "\n").concat(namespace, "\n").concat(JSON.stringify(Object.keys(importsFound))));
        }
        else if (obj === undefined) {
            obj = importedTypes[idx];
        }
        // Create the new ImportedObjectDefinition
        return __assign(__assign({}, obj), { type: namespaceType, __namespaced: true, kind: kind, uri: uri, namespace: namespace, nativeType: type });
    };
    return {
        enter: {
            ObjectRef: function (def) {
                ...
                if (!importsFound[namespaceType]) {
                    if ((0, schema_parse_1.isEnvType)(type)) {
                        ...
                        (0, schema_parse_1.visitImportedEnvDefinition)(importFound, __assign({}, extractObjectImportDependencies(importsFound, rootAbi, namespace, uri)));
                    }
                    else {
                        ...
                        importsFound[importFound.type] = importFound;
                        // Traverse this newly added object
                        (0, schema_parse_1.visitObjectDefinition)(importFound, __assign({}, extractObjectImportDependencies(importsFound, rootAbi, namespace, uri)));
                    }
                }
                return def;
            },
            InterfaceImplementedDefinition: function (def) {
                ...
                if (!importsFound[namespaceType]) {
                    if ((0, schema_parse_1.isEnvType)(type)) {
                        ...
                        importsFound[importFound.type] = importFound;
                        // Traverse this newly added object
                        (0, schema_parse_1.visitEnvDefinition)(importFound, __assign({}, extractObjectImportDependencies(importsFound, rootAbi, namespace, uri)));
                    }
                    else {
                        var importFound = findImport(type, namespaceType, rootAbi.objectTypes || [], rootAbi.importedObjectTypes || [], schema_parse_1.DefinitionKind.ImportedObject);
                        // Keep track of it
...
function appendNamespace(namespace, str) {
    return "".concat(namespace, "_").concat(str);
}
function addModuleImportsDirective(schema, externalImports) {
    ...
    var modifySchema = function () {
        ...
        var importedTypes = "".concat(externalImports
...
function addCapabilityDirective(schema, capabilities) {
    ...
    capabilities.forEach(function (capability) {
        ...
        var replacementModuleStr = "type Module $1@capability(\ntype: \"".concat(capability.type, "\",\nuri: \"").concat(capability.uri, "\",\nnamespace: \"").concat(capability.namespace, "\"\n) {");
...
function parseInterfaces(implementInterfaceStatments) {
    ...
    try {
        for (var implementInterfaceStatments_1 = __values(implementInterfaceStatments), implementInterfaceStatments_1_1 = implementInterfaceStatments_1.next(); !implementInterfaceStatments_1_1.done; implementInterfaceStatments_1_1 = implementInterfaceStatments_1.next()) {
            ...
            var typeCapture = new RegExp("type\\s+(".concat(TYPE_NAME_REGEX, ")\\s+"), "g");
            var typeNameMatches = typeCapture.exec(implementMatch[0]);
            if (!typeNameMatches) {
                continue;
            }
            var typeName = typeNameMatches[1];
            var interfaces = __spreadArray([], __read(implementStr.matchAll(new RegExp("(".concat(TYPE_NAME_REGEX, ")(&s+)*"), "g"))), false).map(function (x) { return x[0]; });
...
function resolveInterfaces(schema, implementationsWithInterfaces) {
    ...
    var getAllUniqueInterfaces = function () {
        ...
        return __spreadArray([], __read(new Set(allIntefaces)), false);
    };
    var allInterfaces = getAllUniqueInterfaces();
    var interfacesWithBodies = [];
    var typeCapture = new RegExp("type\\s+(".concat(TYPE_NAME_REGEX, ")[^{]+{([^}]*)}"), "g");
    var sanitizedSchema = removeComments(schema);
    var typeMatches = __spreadArray([], __read(sanitizedSchema.matchAll(typeCapture)), false);
    ...
    try {
        for (var implementationsWithInterfaces_1 = __values(implementationsWithInterfaces), implementationsWithInterfaces_1_1 = implementationsWithInterfaces_1.next(); !implementationsWithInterfaces_1_1.done; implementationsWithInterfaces_1_1 = implementationsWithInterfaces_1.next()) {
            ...
            var implementationTypeCapture = new RegExp("(type\\s+".concat(implementationWithInterfaces.typeName, "\\s+[^{]*){([^}]*)}"));
            ...
            schema = schema.replace(implementationTypeCapture, "$1{$2".concat(bodiesOfInterfacesStr, "\n}"));
        }
    }
...
function resolveExternalImports(schemaFile, importsToResolve, abiResolver, abi) {
    return __awaiter(this, void 0, void 0, function () {
            ...
            switch (_b.label) {
                case 0:
                    ...
                    _loop_3 = function (importToResolve) {
                        var importFrom, namespace, importedTypes, uri, schemaOrAbi, extAbi, schemaFile_1, extTypesToImport, starIdx, _loop_4, extTypesToImport_1, extTypesToImport_1_1, importedType, _loop_5, _c, _d, importName;
                        var e_6, _e, e_7, _f;
                        return __generator(this, function (_g) {
                            switch (_g.label) {
                                case 0:
                                    importFrom = importToResolve.importFrom, namespace = importToResolve.namespace, importedTypes = importToResolve.importedTypes;
                                    uri = importFrom;
                                    return [4 /*yield*/, abiResolver(importFrom, schemaFile)];
                                case 1:
                                    schemaOrAbi = _g.sent();
                                    extAbi = void 0;
                                    if (!(typeof schemaOrAbi.schema === "string")) return [3 /*break*/, 3];
                                    schemaFile_1 = schemaOrAbi;
                                    return [4 /*yield*/, resolveImportsAndParseSchemas(schemaFile_1.schema, schemaFile_1.absolutePath, abiResolver, true)];
                                case 2:
                                    extAbi = _g.sent();
                                    return [3 /*break*/, 4];
                                case 3:
                                ...
                                case 4:
                                    if (!extAbi) {
                                        throw Error("Unable to resolve abi at \"".concat(importFrom, "\""));
                                    }
                                    ...
                                    if (starIdx > -1) {
                                        ...
                                        if (extAbi.objectTypes) {
                                            extTypesToImport.push.apply(extTypesToImport, __spreadArray([], __read(extAbi.objectTypes.map(function (x) { return x.type; })), false));
                                        }
                                        if (extAbi.enumTypes) {
                                            extTypesToImport.push.apply(extTypesToImport, __spreadArray([], __read(extAbi.enumTypes.map(function (x) { return x.type; })), false));
                                        }
                                    ...
                                    _loop_4 = function (importedType) {
                                        ...
                                        if ((0, schema_parse_1.isModuleType)(importedType)) {
                                            if (!extAbi.moduleType) {
                                                extAbi.moduleType = (0, schema_parse_1.createModuleDefinition)({});
                                            }
                                            extTypes = [extAbi.moduleType];
                                            visitorFunc = schema_parse_1.visitModuleDefinition;
                                            var type = extAbi.moduleType;
                                            trueType = __assign(__assign({}, (0, schema_parse_1.createImportedModuleDefinition)(__assign(__assign({}, type), { required: undefined, uri: uri, nativeType: type.type, namespace: namespace }))), { methods: type.methods });
                                        }
                                        else if ((0, schema_parse_1.isImportedModuleType)(importedType)) {
                                            throw Error("Cannot import an import's imported module type. Tried to import ".concat(importedType, " from ").concat(uri, "."));
                                        }
                                        else if ((0, schema_parse_1.isEnvType)(importedType)) {
                                            if (!extAbi.envType) {
                                                throw new Error("Tried to import env type from ".concat(uri, " but it doesn't exist."));
                                            }
                                            extTypes = [extAbi.envType];
                                            visitorFunc = schema_parse_1.visitEnvDefinition;
                                            var type = extAbi.envType;
                                            trueType = __assign(__assign({}, (0, schema_parse_1.createImportedEnvDefinition)(__assign(__assign({}, type), { name: undefined, required: undefined, uri: uri, nativeType: type.type, namespace: namespace }))), { properties: type.properties });
                                        }
                                        else if ((0, schema_parse_1.isImportedEnvType)(importedType)) {
                                            throw Error("Cannot import an import's imported env type. Tried to import ".concat(importedType, " from ").concat(uri, "."));
                                        }
                                        else {
                                            ...
                                            if (objIdx > -1) {
                                                ...
                                                trueType = __assign(__assign({}, (0, schema_parse_1.createImportedObjectDefinition)(__assign(__assign({}, type), { type: appendNamespace(namespace, importedType), name: undefined, required: undefined, uri: uri, nativeType: type.type, namespace: namespace }))), { properties: type.properties });
                                            }
                                            else if (impObjIdx > -1) {
                                                ...
                                                trueType = (0, schema_parse_1.createImportedEnumDefinition)(__assign(__assign({}, type), { type: appendNamespace(namespace, importedType), uri: uri, nativeType: type.type, namespace: namespace }));
                                            }
                                        ...
                                        if (!trueType) {
                                            throw Error("Cannot find type \"".concat(importedType, "\" in the schema at ").concat(uri, ".\nFound: ").concat(extTypes && JSON.stringify(extTypes.map(function (type) { return type.type; }))));
                                        }
                                        if (!visitorFunc) {
                                    ...
                                    try {
                                        ...
                                        try {
                                            if (extTypesToImport_1_1 && !extTypesToImport_1_1.done && (_e = extTypesToImport_1.return)) _e.call(extTypesToImport_1);
                                        }
                                        ...
                                        if (importType.kind === schema_parse_1.DefinitionKind.ImportedEnv) {
                                            ...
                                            append = function () {
                                                ...
                                                abi.importedEnvTypes &&
                                                    abi.importedEnvTypes.push((0, schema_parse_1.visitImportedEnvDefinition)(importDef, namespaceTypes(namespace)));
                                            };
                                        }
                                            ...
                                            append = function () {
                                                ...
                                                abi.importedObjectTypes &&
                                                    abi.importedObjectTypes.push((0, schema_parse_1.visitImportedObjectDefinition)(importDef, namespaceTypes(namespace)));
                                            };
                                        }
                                            ...
                                            append = function () {
                                                var importDef = importType;
                                                abi.importedModuleTypes &&
                                                    abi.importedModuleTypes.push((0, schema_parse_1.visitImportedModuleDefinition)(importDef, namespaceTypes(namespace)));
                                            };
                                        }
                                            ...
                                            append = function () {
                                                abi.importedEnumTypes &&
                                                    abi.importedEnumTypes.push((0, schema_parse_1.visitImportedEnumDefinition)(importType, namespaceTypes(namespace)));
                                            };
                                        }
                                        else {
                                            throw Error("resolveExternalImports: This should never happen, unknown kind.\n".concat(JSON.stringify(importType, null, 2)));
                                        }
                                    ...
                                    try {
                                        ...
                                        for (_c = (e_7 = void 0, __values(Object.keys(typesToImport))), _d = _c.next(); !_d.done; _d = _c.next()) {
                                            importName = _d.value;
                                            _loop_5(importName);
                                        }
                                    }
...
function resolveLocalImports(schemaFile, importsToResolve, abiResolver, abi) {
    return __awaiter(this, void 0, void 0, function () {
            ...
            switch (_b.label) {
                case 0:
                    _loop_6 = function (importToResolve) {
                        var importedTypes, importFrom, schemaOrAbi, localAbi, schemaFile_2, extTypesToImport, starIdx, typesToImport, _loop_7, extTypesToImport_2, extTypesToImport_2_1, importedType, _loop_8, _c, _d, importType;
                        var e_9, _e, e_10, _f;
                        return __generator(this, function (_g) {
                            switch (_g.label) {
                                case 0:
                                    importedTypes = importToResolve.importedTypes, importFrom = importToResolve.importFrom;
                                    return [4 /*yield*/, abiResolver(importFrom, schemaFile)];
                                case 1:
                                    schemaOrAbi = _g.sent();
                                    if (!(typeof schemaOrAbi.schema === "string")) return [3 /*break*/, 3];
                                    schemaFile_2 = schemaOrAbi;
                                    return [4 /*yield*/, resolveImportsAndParseSchemas(schemaFile_2.schema, schemaFile_2.absolutePath, abiResolver, true)];
                                case 2:
                                    localAbi = _g.sent();
                                    return [3 /*break*/, 4];
                                case 3:
                                    localAbi = schemaOrAbi;
                                    _g.label = 4;
                                case 4:
                                    if (!localAbi) {
                                        throw Error("Unable to resolve schema at \"".concat(importFrom, "\""));
                                    }
                                    extTypesToImport = importedTypes;
                                    starIdx = extTypesToImport.indexOf("*");
                                    // If the importedTypes array contains the catch-all "*"
                                    ...
                                    if (starIdx > -1) {
                                        ...
                                        if (localAbi.objectTypes) {
                                            extTypesToImport.push.apply(extTypesToImport, __spreadArray([], __read(localAbi.objectTypes.map(function (x) { return x.type; })), false));
                                        }
                                        if (localAbi.enumTypes) {
                                            extTypesToImport.push.apply(extTypesToImport, __spreadArray([], __read(localAbi.enumTypes.map(function (x) { return x.type; })), false));
                                        }
                                    ...
                                    _loop_7 = function (importedType) {
                                        ...
                                        if (!type) {
                                            throw Error("Cannot find type \"".concat(importedType, "\" in the schema at ").concat(importFrom, ".\nFound: [ ").concat(localAbi.objectTypes &&
                                        ...
                                        var findImport = function (def, rootTypes) {
                                            ...
                                            if (idx === -1) {
                                                throw Error("resolveLocalImports: Cannot find the requested type within the Abi.\n" +
                                                    "Type: ".concat(def.type, "\nAbi: ").concat(JSON.stringify(localAbi)));
                                            }
                                            ...
                                            visitorFunc(type, {
                                                enter: {
                                                    ObjectRef: function (def) {
                                                        var allObjectTypes = [];
                                                        if (localAbi.objectTypes) {
                                                            allObjectTypes.push.apply(allObjectTypes, __spreadArray([], __read(localAbi.objectTypes), false));
                                                        }
                                                        if (localAbi.importedObjectTypes) {
                                                            allObjectTypes.push.apply(allObjectTypes, __spreadArray([], __read(localAbi.importedObjectTypes), false));
                                                        }
                                                    ...
                                                    EnumRef: function (def) {
                                                        var allEnumTypes = [];
                                                        if (localAbi.enumTypes) {
                                                            allEnumTypes.push.apply(allEnumTypes, __spreadArray([], __read(localAbi.enumTypes), false));
                                                        }
                                                        if (localAbi.importedEnumTypes) {
                                                            allEnumTypes.push.apply(allEnumTypes, __spreadArray([], __read(localAbi.importedEnumTypes), false));
                                                        }
                                    ...
                                    try {
                                        ...
                                        try {
                                            if (extTypesToImport_2_1 && !extTypesToImport_2_1.done && (_e = extTypesToImport_2.return)) _e.call(extTypesToImport_2);
                                        }
                                        finally { if (e_9) throw e_9.error; }
                                    }
                                    _loop_8 = function (importType) {
                                        var _h;
                                        if ((0, schema_parse_1.isKind)(typesToImport[importType], schema_parse_1.DefinitionKind.Module)) {
                                            abi.moduleType = typesToImport[importType];
                                        }
                                        else if ((0, schema_parse_1.isKind)(typesToImport[importType], schema_parse_1.DefinitionKind.Env)) {
                                            if (!abi.envType) {
                                                abi.envType = (0, schema_parse_1.createEnvDefinition)({});
                                            }
                                            var sharedEnv = localAbi.envType;
                                            if (sharedEnv.properties) {
                                                (0, env_1.checkDuplicateEnvProperties)(abi.envType, sharedEnv.properties);
                                                if (abi.envType.properties) {
                                                    (_h = abi.envType.properties).push.apply(_h, __spreadArray([], __read(sharedEnv.properties), false));
                                                }
                                                else {
                                        ...
                                        else if ((0, schema_parse_1.isKind)(typesToImport[importType], schema_parse_1.DefinitionKind.ImportedObject)) {
                                        ...
                                        else if ((0, schema_parse_1.isKind)(typesToImport[importType], schema_parse_1.DefinitionKind.Object)) {
                                            if (abi.objectTypes &&
                                                abi.objectTypes.findIndex(function (def) { return def.type === importType; }) === -1) {
                                                abi.objectTypes.push(typesToImport[importType]);
                                            }
                                        }
                                        else if ((0, schema_parse_1.isKind)(typesToImport[importType], schema_parse_1.DefinitionKind.ImportedEnum)) {
                                        ...
                                        else if ((0, schema_parse_1.isKind)(typesToImport[importType], schema_parse_1.DefinitionKind.Enum)) {
                                            if (abi.enumTypes &&
                                    ...
                                    try {
                                        // Add all imported types into the aggregate Abi
                                        for (_c = (e_10 = void 0, __values(Object.keys(typesToImport))), _d = _c.next(); !_d.done; _d = _c.next()) {
                                            importType = _d.value;
                                            _loop_8(importType);
                                        }
                                    }
