...
function applyRequired(type, required) {
    return "".concat(type).concat(required ? "!" : "");
}
...
function toGraphQL(def, prefixed) {
    ...
    switch (def.kind) {
        ...
        case __1.DefinitionKind.ImportedEnum:
            if (prefixed) {
                return applyRequired("Enum_".concat(def.type), def.required);
            }
            return applyRequired(def.type, def.required);
        ...
        case __1.DefinitionKind.Array: {
            var array = def;
            if (!array.item) {
                throw Error("toGraphQL: ArrayDefinition's item type is undefined.\n".concat(JSON.stringify(array, null, 2)));
            }
            return applyRequired("[".concat(toGraphQL(array.item, prefixed), "]"), array.required);
        }
        case __1.DefinitionKind.Map: {
            var map = def;
            if (!map.key) {
                throw Error("toGraphQL: MapDefinition's key type is undefined.\n".concat(JSON.stringify(map, null, 2)));
            }
            if (!map.value) {
                throw Error("toGraphQL: MapDefinition's value type is undefined.\n".concat(JSON.stringify(map, null, 2)));
            }
            return applyRequired("Map<".concat(toGraphQL(map.key, prefixed), ", ").concat(toGraphQL(map.value, prefixed), ">"), map.required);
        }
        case __1.DefinitionKind.Method: {
            var method = def;
            if (!method.return) {
                throw Error("toGraphQL: MethodDefinition's return type is undefined.\n".concat(JSON.stringify(method, null, 2)));
            }
            var result = "".concat(method.name, "(\n  ").concat((method.arguments || [])
                .map(function (arg) { return "".concat(arg.name, ": ").concat(toGraphQL(arg, prefixed)); })
                .join("\n    "), "\n): ").concat(toGraphQL(method.return, prefixed));
            return result;
        }
