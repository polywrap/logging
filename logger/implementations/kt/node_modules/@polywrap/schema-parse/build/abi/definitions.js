...
function createMapKeyDefinition(args) {
    if (!(0, utils_1.isMapKeyType)(args.type)) {
        throw Error("createMapKeyDefinition: Unrecognized Map key type provided \"".concat(args.type, "\""));
    }
...
function createScalarDefinition(args) {
    if (!(0, utils_1.isScalarType)(args.type)) {
        throw Error("createScalarDefinition: Unrecognized scalar type provided \"".concat(args.type, "\""));
    }
    return __assign(__assign({}, args), { kind: DefinitionKind.Scalar });
}
...
function createMethodDefinition(args) {
    var _a, _b;
    if (((_a = args.name) === null || _a === void 0 ? void 0 : _a.startsWith("__")) && ((_b = args.name) === null || _b === void 0 ? void 0 : _b.endsWith("__"))) {
        throw new Error("Dunder methods (Methods starting and ending with __) are reserved methods");
    }
...
function createImportedModuleDefinition(args) {
    if (!(0, utils_1.isModuleType)(args.nativeType)) {
        throw Error("createImportedModuleDefinition: Unrecognized module type provided \"".concat(args.nativeType, "\""));
    }
    return __assign(__assign(__assign({}, args), createGenericDefinition(__assign(__assign({}, args), { type: "".concat(args.namespace, "_").concat(args.nativeType) }))), { kind: DefinitionKind.ImportedModule });
}
...
function createImportedEnvDefinition(args) {
    return __assign(__assign(__assign({}, args), createObjectDefinition(__assign(__assign({}, args), { type: "".concat(args.namespace, "_Env") }))), { kind: DefinitionKind.ImportedEnv });
}
