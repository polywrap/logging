    ...
    if (method && argument) {
        if (!argument.name) {
            throw Error("extractNamedType: Invalid state. Uninitialized currentArgument, name not found.\n" +
                "Argument: ".concat(JSON.stringify(argument, null, 2), "\nState: ").concat(JSON.stringify(state, null, 2)));
        }
        // Argument value
        (0, property_utils_1.setPropertyType)(argument, argument.name, {
        ...
        if (!method.name) {
            throw Error("extractNamedType: Invalid state. Uninitialized currentMethod, name not found.\n" +
                "Method: ".concat(JSON.stringify(method, null, 2), "\nState: ").concat(JSON.stringify(state, null, 2)));
        }
        if (state.currentReturn) {
            (0, property_utils_1.setPropertyType)(state.currentReturn, method.name, {
                type: node.name.value,
                required: state.nonNullType,
            });
        }
...
function extractListType(state) {
    ...
    if (method && argument) {
        // Argument value
        argument.array = (0, __1.createArrayDefinition)({
            name: argument.name,
            type: "N/A",
            required: state.nonNullType,
        });
        ...
        // Return value
        if (!method.return) {
            method.return = (0, __1.createPropertyDefinition)({
                type: "N/A",
                name: method.name,
            });
            state.currentReturn = method.return;
        ...
        state.currentReturn.array = (0, __1.createArrayDefinition)({
            type: "N/A",
            name: method.name,
            required: state.nonNullType,
        });
...
function extractInputValueDefinition(node, state) {
    ...
    var argument = (0, __1.createPropertyDefinition)({
        type: type ? type : "N/A",
        name: name,
            ...
            for (var _d = __values(node.directives), _e = _d.next(); !_e.done; _e = _d.next()) {
                ...
                if (dir.name.value === "env") {
                    ...
                    if (required === undefined) {
                        throw new Error("Env directive: ".concat(node.name.value, " has invalid arguments"));
                    }
                    return {
                        required: required,
                    };
