...
var visitorEnter = function (abi, state) { return ({
    ObjectTypeDefinition: function (node) {
        ...
        var interfaces = (_a = node.interfaces) === null || _a === void 0 ? void 0 : _a.map(function (x) {
            return (0, __1.createInterfaceImplementedDefinition)({ type: x.name.value });
        });
        var module = (0, __1.createModuleDefinition)({
    ...
    FieldDefinition: function (node) {
        ...
        var _b = (0, object_types_utils_1.extractAnnotateDirective)(node, name), type = _b.type, def = _b.def;
        var returnType = (0, __1.createPropertyDefinition)({
            type: type ? type : "N/A",
        ...
        var method = (0, __1.createMethodDefinition)({
            name: node.name.value,
            return: returnType,
            comment: (_a = node.description) === null || _a === void 0 ? void 0 : _a.value,
        });
        var envDirDefinition = (0, module_types_utils_1.extractEnvDirective)(node);
        if (envDirDefinition) {
    ...
    InputValueDefinition: function (node) {
        (0, module_types_utils_1.extractInputValueDefinition)(node, state);
    },
    NonNullType: function (_node) {
        state.nonNullType = true;
    },
    NamedType: function (node) {
        (0, module_types_utils_1.extractNamedType)(node, state);
    },
    ListType: function (_node) {
        (0, module_types_utils_1.extractListType)(state);
    },
}); };
var parseCapabilitiesDirective = function (nodeName, node) {
    ...
    try {
        for (var _b = __values(node.directives), _c = _b.next(); !_c.done; _c = _b.next()) {
            ...
            if (!dir.arguments) {
                throw Error("@capability directive is incomplete, missing arguments. See type ".concat(nodeName, "."));
            }
            var typeIndex = dir.arguments.findIndex(function (arg) { return arg.name.value === "type"; });
            if (typeIndex === -1) {
                throw Error("@capability directive missing required argument \"type\". See type ".concat(nodeName, "."));
            }
            var typeArg = dir.arguments[typeIndex];
            if (typeArg.value.kind !== "StringValue") {
                throw Error("@capability directive's \"type\" argument must be a String type. See type ".concat(nodeName, "."));
            }
            if (!__1.capabilityTypes.includes(typeArg.value.value)) {
                throw Error("@capability directive's \"type\" argument must be one from ".concat(JSON.stringify(__1.capabilityTypes), ". See type ").concat(nodeName, "."));
            }
            var capabilityType = typeArg.value.value;
            var uriIndex = dir.arguments.findIndex(function (arg) { return arg.name.value === "uri"; });
            if (uriIndex === -1) {
                throw Error("@capability directive missing required argument \"uri\". See type ".concat(nodeName, "."));
            }
            var uriArg = dir.arguments[uriIndex];
            if (uriArg.value.kind !== "StringValue") {
                throw Error("@capability directive's \"uri\" argument must be a String type. See type ".concat(nodeName, "."));
            }
            var uri = uriArg.value.value;
            var namespaceIndex = dir.arguments.findIndex(function (arg) { return arg.name.value === "namespace"; });
            if (namespaceIndex === -1) {
                throw Error("@capability directive missing required argument \"namespace\". See type ".concat(nodeName, "."));
            }
            var namespaceArg = dir.arguments[namespaceIndex];
            if (namespaceArg.value.kind !== "StringValue") {
                throw Error("@capability directive's \"namespace\" argument must be a String type. See type ".concat(nodeName, "."));
            }
            var namespace = namespaceArg.value.value;
            if (!interfacesByNamespace[namespace]) {
                interfacesByNamespace[namespace] = (0, __1.createInterfaceDefinition)({
                    type: namespace,
                    uri: uri,
                    namespace: namespace,
                    capabilities: (0, __1.createCapability)({
                        type: capabilityType,
                        enabled: true,
                    }),
                });
...
var parseImportsDirective = function (nodeName, node) {
    ...
    if (importsIndex !== -1) {
        ...
        if (!importsDir.arguments) {
            throw Error("@imports directive is incomplete, missing arguments. See type ".concat(nodeName, "."));
        }
        var typesIndex = importsDir.arguments.findIndex(function (arg) { return arg.name.value === "types"; });
        if (typesIndex === -1) {
            throw Error("@imports directive missing required argument \"types\". See type ".concat(nodeName, "."));
        }
        var typesArg = importsDir.arguments[typesIndex];
        if (typesArg.value.kind !== "ListValue") {
            throw Error("@imports directive's \"types\" argument must be a List type. See type ".concat(nodeName, "."));
        }
        var listValue = typesArg.value;
        listValue.values.forEach(function (value) {
            if (value.kind !== "StringValue") {
                throw Error("@imports directive's \"types\" list must only contain strings. See type ".concat(nodeName, "."));
            }
...
var visitorLeave = function (abi, state) { return ({
    ObjectTypeDefinition: function (_node) {
        ...
        if (state.currentInterfaces) {
            abi.interfaceTypes = __spreadArray(__spreadArray([], __read(abi.interfaceTypes), false), __read(state.currentInterfaces), false);
        }
