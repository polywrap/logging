        ...
        try {
            for (var _d = __values(node.directives), _e = _d.next(); !_e.done; _e = _d.next()) {
                ...
                if (dir.name.value === "annotate") {
                    ...
                    if (!type) {
                        throw new Error("Annotate directive: ".concat(node.name.value, " has invalid arguments"));
                    }
                    def = (0, map_utils_1.parseMapType)(type, name);
                }
            }
        }
...
function extractFieldDefinition(node, state) {
    ...
    if (node.arguments && node.arguments.length > 0) {
        throw Error("Imported types cannot have methods. See type \"".concat(importDef.name, "\""));
    }
    var name = node.name.value;
    var _b = extractAnnotateDirective(node, name), type = _b.type, def = _b.def;
    var property = (0, __1.createPropertyDefinition)({
        type: type ? type : "N/A",
        name: name,
...
function extractNamedType(node, state) {
    ...
    if (!property.name) {
        throw Error("extractNamedType: Invalid state. Uninitialized currentProperty, name not found.\n" +
            "Method: ".concat(JSON.stringify(property, null, 2), "\nState: ").concat(JSON.stringify(state, null, 2)));
    }
    (0, property_utils_1.setPropertyType)(property, property.name, {
...
function extractListType(state) {
    ...
    property.array = (0, __1.createArrayDefinition)({
        name: property.name,
        type: "N/A",
        required: state.nonNullType,
    });
