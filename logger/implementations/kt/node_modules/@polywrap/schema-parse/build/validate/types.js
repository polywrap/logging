            ...
            enter: {
                ...
                InterfaceTypeDefinition: function (node) {
                    throw Error("Interface type definitions are not supported.\n" +
                        "Found: interface ".concat(node.name.value, " { ... }\n") +
                        "Please Use: type ".concat(node.name.value, " { ... }"));
                },
                // No Inputs
                InputObjectTypeDefinition: function (node) {
                    throw Error("Input type definitions are not supported.\n" +
                        "Found: input ".concat(node.name.value, " { ... }\n") +
                        "Please Use: type ".concat(node.name.value, " { ... }"));
                },
                ObjectTypeDefinition: function (node) {
                    // No Operation types
                    if (operationTypeNames.includes(node.name.value)) {
                        throw Error("OperationType names (".concat(operationTypeNames.join(", "), ") are not allowed."));
                    }
                    // No duplicates
                    if (objectTypes[node.name.value]) {
                        throw Error("Duplicate object type definition found: ".concat(node.name.value));
                    }
                    objectTypes[node.name.value] = true;
                },
                // No New Scalars
                ScalarTypeDefinition: function (node) {
                    if (node.name.value !== "Map" && !(0, __1.isScalarType)(node.name.value)) {
                        throw Error("Custom scalar types are not supported. Found: \"".concat(node.name.value, "\". Supported scalars: ").concat(__1.scalarTypeNames));
                    }
                },
                // No Unions
                UnionTypeDefinition: function (node) {
                    throw Error("Union type definitions are not supported.\n" +
                        "Found: union ".concat(node.name.value));
                },
            },
        },
    };
            ...
            enter: {
                ...
                InputValueDefinition: function (node) {
                    var typeName = currentImportType
                        ? currentImportType
                        : currentObject;
                    if (typeName && !(0, __1.isModuleType)(typeName)) {
                        // Arguments not supported on non-module types
                        throw Error("Methods can only be defined on module types (Module).\n" +
                            "Found: type ".concat(typeName, " { ").concat(currentField, "(").concat(node.name.value, ") }"));
                    }
                },
            },
            leave: {
        ...
        cleanup: function () {
            ...
            try {
                ...
                for (var fieldTypes_1 = __values(fieldTypes), fieldTypes_1_1 = fieldTypes_1.next(); !fieldTypes_1_1.done; fieldTypes_1_1 = fieldTypes_1.next()) {
                    ...
                    if (!(0, __1.isScalarType)(field.type) &&
                        !objectTypes[field.type] &&
                        !enumTypes[field.type] &&
                        field.type !== "Map") {
                        throw Error("Unknown property type found: type ".concat(field.object, " { ").concat(field.field, ": ").concat(field.type, " }"));
                    }
                }
            }
            ...
            if (objectTypeNames.length) {
                throw new Error("Found duplicate fields in the following objects:".concat(objectTypeNames.map(function (object) {
                    return "\ntype ".concat(object, " => ").concat(JSON.stringify(Object.keys(duplicateFields[object])));
                })));
            }
        },
    };
