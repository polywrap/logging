    ...
    return {
        ...
        cleanup: function () {
            if (unsupportedUsages.length) {
                throw new Error("Found the following usages of unsupported directives:".concat(unsupportedUsages.map(function (u) { return "\n@".concat(u); })));
            }
        },
    };
};
        ...
        visitor: {
            enter: {
                ...
                FieldDefinition: function (node) {
                    ...
                    if (envDirective &&
                        currentType &&
                        !(0, __1.isModuleType)(currentType) &&
                        !(0, __1.isImportedModuleType)(currentType)) {
                        throw new Error("@env directive should only be used on Module method definitions. Found on field '".concat(node.name.value, "' of type '").concat(currentType, "'"));
                    }
                },
            },
            leave: {
...
var getImportsDirectiveValidator = function () {
    ...
    var ObjectTypeDefinition = function (node) {
        ...
        if (badUsageLocations.length) {
            throw new Error("@imports directive should only be used on Module type definitions, " +
                "but it is being used on the following ObjectTypeDefinitions:".concat(badUsageLocations.map(function (b) { return "\n".concat(b); })));
        }
    };
    var Directive = function (node, key, parent, path) {
        ...
        if (!isInsideObjectTypeDefinition) {
            throw new Error("@imports directive should only be used on Module type definitions, " +
                "but it is being used in the following location: ".concat(path.join(" -> ")));
        }
        ...
        if (args.length > 1) {
            throw new Error("@imports directive takes only one argument 'types', but found: ".concat(args
...
var getImportedDirectiveValidator = function () {
    ...
    var Directive = function (node, key, parent, path) {
        ...
        if (missingArguments.length) {
            throw new Error("@imported directive is missing the following arguments:".concat(missingArguments.map(function (arg) { return "\n- ".concat(arg); })));
        }
        ...
        if (extraArguments.length) {
            throw new Error("@imported directive takes only 3 arguments: ".concat(expectedArguments.join(", "), ". But found:").concat(extraArguments.map(function (arg) { return "\n- ".concat(arg); })));
        }
    };
    return {
        visitor: {
