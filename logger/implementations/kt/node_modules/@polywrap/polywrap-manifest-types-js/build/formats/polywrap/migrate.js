...
function migratePolywrapManifest(manifest, to, logger) {
    ...
    if (!(Object.values(_1.PolywrapManifestFormats).some(function (x) { return x === from; }))) {
        throw new Error("Unrecognized PolywrapManifestFormat \"".concat(manifest.format, "\""));
    }
    if (!(Object.values(_1.PolywrapManifestFormats).some(function (x) { return x === to; }))) {
        throw new Error("Unrecognized PolywrapManifestFormat \"".concat(to, "\""));
    }
    var migrationPath = (0, migrations_1.findShortestMigrationPath)(migrators_1.migrators, from, to);
    if (!migrationPath) {
        throw new Error("Migration path from PolywrapManifestFormat \"".concat(from, "\" to \"").concat(to, "\" is not available"));
    }
