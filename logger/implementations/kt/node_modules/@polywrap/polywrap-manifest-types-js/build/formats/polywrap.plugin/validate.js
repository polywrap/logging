...
var _0_3_0_json_1 = __importDefault(require("@polywrap/polywrap-manifest-schemas/formats/polywrap.plugin/0.3.0.json"));
var _0_4_0_json_1 = __importDefault(require("@polywrap/polywrap-manifest-schemas/formats/polywrap.plugin/0.4.0.json"));
...
function validatePluginManifest(manifest, extSchema) {
    ...
    if (!schema) {
        throw Error("Unrecognized PluginManifest schema format \"".concat(manifest.format, "\"\nmanifest: ").concat(JSON.stringify(manifest, null, 2)));
    }
    var throwIfErrors = function (result) {
        if (result.errors.length) {
            throw new Error(__spreadArray([
                "Validation errors encountered while sanitizing PluginManifest format ".concat(manifest.format)
